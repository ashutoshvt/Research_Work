...
under the Born-Oppenheimer approximation\cite{} and non-relativistic regimes\cite{}. This
...
Configuration interaction (CI) method is a matrix based formulation of the time 
independent Schr\"odinger's equation,
\begin{equation}
HC = EC
\end{equation}
where H is matrix representation of the Hamiltonian , C is the coefficient matrix and 
E is the diagonal matrix containing the electronic energies (The lowest energy 
corresponds to the ground state). If the vector space used to represent the Hamiltonian 
...
composed of substituted or excited slater determiants which besides being complete 
also have the desired properties of antisymmetry and orthogonality. 
As such, the CI wavefunction is a linear combination of slater determinants.
\\
\begin{equation}
...
and so on.\\
Full CI method is computationally very expensive and scales factorially with 
system size. Unsurprsingly, it is mostly used in benchmark calculations of small molecules and
the largest Full CI calculation to date has been on nitrogen molecule\cite{} which involved 
billions of determinants. In practice, truncated CI methods like CISD, CISDT, etc.,
where the vector space is restricted to include up to doubly and triply excited determinants 
and so on is used. However, unlike the exact wavefunction, truncated CI wavefunctions 
do not posess the property of size-extensivity i.e. the CISD energy doesn't scale 
linearly with the number of electrons in the asymptotic limit. As a result,
the accuracy of these methods takes a hit as the size of the system increases.
