...
choice of how to construct perturbed densities now as one can have either 
$\sum\limits_q \frac{1}{3}[{D^{\mu_q}_{ab}}]^{(2)}$ or $\sum\limits_q\frac{1}{3}[{D^{p_q}_{ab}}]^{(2)}$.
...
in VG calculations is the replacement of $\sum\limits_q \frac{1}{3}[{D^{\mu_q}_{ab}}]^{(2)}$ by 
$\sum\limits_q \frac{1}{3}[{D^{p_q}_{ab}}]^{(2)}$ where $p$ is the momentum operator, i.e. 
$p_q = -i\frac{\partial}{\partial{q}}$. We report the performances of all the three approaches
