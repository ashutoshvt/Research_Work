...
of equations involving the CC Jacobian ($\bar{H}_{\mu\nu} = \langle \mu | \big[\bar{H},\tau_\nu\big] |0\rangle$),
\\
\begin{equation}
\begin{split}
...
for correlated response calculations. For a perturbation operator A, 
\begin{equation}
...
perturbed densities for such calculations is : $\sum\limits_q \frac{1}{3}{D^{'}(\mu_q)}^{(2)}$,
where $q \equiv [x,y,z]$ . 
\subsection{Specific Rotations}
For specific rotation calculations, we employed both the LG and velocity-gauge (VG) 
representations. If we just consider the LG calculations for now, there is no obvious
choice of how to construct perturbed densities now. One can have either 
$\sum\limits_q \frac{1}{3}{D^{'}(\mu_q)}^{(2)}$ or $\sum\limits_q\frac{1}{3}{D^{'}(m_q)}^{(2)}$.
Another possible option is to diagonalize both the densities separately, truncate the VNOs,
and then form the virtual space by taking a union of the truncated VNOs. The only difference
in VG calculations is the replacement of $\sum\limits_q \frac{1}{3}{D^{'}(\mu_q)}^{(2)}$ by 
by $\sum\limits_q \frac{1}{3}{D^{'}(p_q)}^{(2)}$ where $p_q = -i\frac{\partial}{\partial{q}}$.
We report the performances of all the three approaches.\\ \\
The primary basis set used in this work is aug-cc-pVDZ basis
...
solvated molecular clusters using DF-MP2 which can give us an estimate of 
the computational savings that can be exploited. It should be noted that 
we have used the same virtual space for calculating both unperturbed 
