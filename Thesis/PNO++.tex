...
regard\cite{Pulay83,Saebo86,PulaySaebo93}. As the name suggests, PAOs are formed by projecting out the occupied MO components from the 
...
ground-state properties like reaction enthalpies, thermodynamic constants etc.\cite{Hampel96, Schutz00a,Pulay83,Saebo86,PulaySaebo93,Schutz00b,Schutz01}. Crawford and King 
...
localization errors for their test set to only 0.06 eV. Sch{\"u}tz and co-workers extended this approach with 
...
average errors of 1.61\% and 0.48\% (with respect to full canonical result) respectively. Further minimization of the errors required building bigger domains leading to a significant increase in computational expenses. In the same year, Russ and Crawford\cite{Russ04} 
used a modified domain building procedure within the PAO framework, where they augmented the ground state 
orbital domains on the basis of first-order orbital response coefficients obtained by solving the coupled-perturbed 
...
\cite{NeeseCCSD09,Neese09}. They also proposed a variant of the LPNO approach, a scheme which they call domain based LPNO (DLPNO), 
where the PNOs are expressed in terms of PAOs, to achieve near-linear scaling behavior in MP2 and CC calculations
\cite{Riplinger16}. Following their pioneering works, the LPNO and the DLPNO methods have been used quite sucessfully to study 
...
transition orbitals and natural orbitals for calculating excitation energies have also been reported\cite{Baudin17,HofenerKlopper17,Mester17,Mester18}. 
...
the regular method.
% for the same test systems as our earlier PNO paper.
\section{Theory}
...
spaces of different pairs of LMOs are non-orthogonal to each other and the transformation
...
and specific rotations. This is unsurprising since the sparsity patterns in the perturbed 
...
PNO-MP2 and PNO-CIS(D) densities for ground and excited state calculations respectively\cite{Hattig18,Hattig13}.
...
schemes could be explored where a) PNOs are defined from perturbed densities, and  b) effective PNOs 
...
be shown that on replacing the CC ground state amplitudes with those of MP2 in CC-LR theory
...
%from the density-fitted 
%3-centered integrals. 
...
We use linear (P)-$(H_2)_n$ helices as our test cases as these are ideal systems for demonstrating 
the efficiency of the local correlation approaches.
%mostly focus on small chiral systems of varying topologies: (linear) (P)-$(H_2)_n$ helices and (M)-1-Fluoralkanes, 
...
formalism with the aug-cc-pVDZ (aDZ)\cite{Dunning89,Kendall92,Woon94} basis set at 589 nm using Psi4 and Psi4Numpy software packages\cite{Parrish17,Smith18}. Furthermore, the electric dipole operator $\mu$ is represented in length gauge (LG) for polarizabilities
while specific rotations employ both LG and modified velocity gauge (MVG) representations. 
Also, we have chosen electric dipole based perturbed pair density for defining
our PNO space:  %In this approach, an average of the density corresponding to this operator over all cartesian
%compoments to define our virtual space. i,e. 
$ {\bm{D}^{\mu;ij}} = \sum\limits_q \frac{1}{3}[{\bm{D}^{\mu_q;ij}}]$, where $q \equiv [x,y,z]$.
%polarizabilities are pretty straightforward as its LRF only involves
...
Through our earlier works\cite{Kumar18:1,Kumar17}, we have demonstrated the poor performances of 
...
which are crucial to the description of the low lying Rydberg states and hence to
the response properties, have very low contributions to the ground state correlation energy
and will always be removed first when ground state densities are used
...
approach. Also, the selection of the diffuse space is not straightforward as the criterion of 
...
the wavefunction retained in these calculations. Thus, a T$_2$ ratio of 1
...
%the PNO scheme and converges to the full result at a T$_22$ ratio slightly greater 
...
%For example, the error in the PNO approach at a T$_2$ ratio of 0.2 is an order of
...
%to move farther away from the full result up to T$_2$ ratios close to 0.6. Thereafter, it slowly 
%converges towards the canonical result. On the other hand, the PNO++ curve has the right
%convergence behavior: as the values of T$_2$ ratios increase, it converges faster to the full result.
...
%after T$_2$ ratios higher than 0.8 is achieved whereas the PNO++ graph starts converging
